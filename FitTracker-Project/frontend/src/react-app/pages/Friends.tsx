...
export default function Friends() {
    ...
    const handleApiAction = async (action: () => Promise<unknown>, successMessage: string, errorMessage: string) => {
                ...
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} className="bg-white/60 backdrop-blur-xl rounded-2xl shadow-lg border p-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            {incoming.length === 0 ? <p className="text-gray-500 text-sm">No incoming requests</p> : <div className="space-y-3">{incoming.map((u) => (<div key={u._id} className="flex items-center justify-between bg-gray-50 rounded-lg p-3"><div className="flex items-center space-x-3"><img src={u.image || `https://ui-avatars.com/api/?name=${u.displayName || u.email}`} alt={u.displayName} className="w-10 h-10 rounded-full" /><div><p className="font-medium text-gray-900">{u.displayName || u.email}</p></div></div><div className="flex items-center space-x-2"><button onClick={() => acceptRequest(u._id)} className="px-3 py-1.5 bg-blue-600 text-white rounded-md text-sm hover:bg-blue-700">Accept</button><button onClick={() => declineRequest(u._id)} className="px-3 py-1.5 bg-gray-200 text-gray-700 rounded-md text-sm hover:bg-gray-300">Decline</button></div></div>))}</div>}
                        </div>
                        <div>
                            <h3 className="font-semibold text-gray-800 mb-3">Outgoing ({outgoing.length})</h3>
                            {outgoing.length === 0 ? <p className="text-gray-500 text-sm">No outgoing requests</p> : <div className="space-y-3">{outgoing.map((u) => (<div key={u._id} className="flex items-center justify-between bg-gray-50 rounded-lg p-3"><div className="flex items-center space-x-3"><img src={u.image || `https://ui-avatars.com/api/?name=${u.displayName || u.email}`} alt={u.displayName} className="w-10 h-10 rounded-full" /><div><p className="font-medium text-gray-900">{u.displayName || u.email}</p></div></div><div className="flex items-center space-x-2"><button onClick={() => cancelOutgoing(u._id)} className="px-3 py-1.5 bg-red-100 text-red-700 rounded-md text-sm hover:bg-red-200">Cancel</button></div></div>))}</div>}
                        </div>
                    </div>
                </motion.div>
