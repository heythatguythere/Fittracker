        ...
        <Layout>
            <div className="space-y-8">
                ...
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} className="bg-white/60 backdrop-blur-xl rounded-2xl shadow-lg border p-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            {incoming.length === 0 ? <p className="text-gray-500 text-sm">No incoming requests</p> : <div className="space-y-3">{(Array.isArray(incoming) ? incoming : []).map((u) => (<div key={u._id} className="flex items-center justify-between bg-gray-50 rounded-lg p-3"><div className="flex items-center space-x-3"><img src={u.image || `https://ui-avatars.com/api/?name=${u.displayName || u.email}`} alt={u.displayName || u.email} className="w-10 h-10 rounded-full" /><div><p className="font-medium text-gray-900">{u.displayName || u.email}</p></div></div><div className="flex items-center space-x-2"><button onClick={() => acceptRequest(u._id)} className="px-3 py-1.5 bg-blue-600 text-white rounded-md text-sm hover:bg-blue-700">Accept</button><button onClick={() => declineRequest(u._id)} className="px-3 py-1.5 bg-gray-200 text-gray-700 rounded-md text-sm hover:bg-gray-300">Decline</button></div></div>))}</div>}
                        </div>
                        <div>
                            <h3 className="font-semibold text-gray-800 mb-3">Outgoing ({outgoing.length})</h3>
                            {outgoing.length === 0 ? <p className="text-gray-500 text-sm">No outgoing requests</p> : <div className="space-y-3">{(Array.isArray(outgoing) ? outgoing : []).map((u) => (<div key={u._id} className="flex items-center justify-between bg-gray-50 rounded-lg p-3"><div className="flex items-center space-x-3"><img src={u.image || `https://ui-avatars.com/api/?name=${u.displayName || u.email}`} alt={u.displayName || u.email} className="w-10 h-10 rounded-full" /><div><p className="font-medium text-gray-900">{u.displayName || u.email}</p></div></div><div className="flex items-center space-x-2"><button onClick={() => cancelOutgoing(u._id)} className="px-3 py-1.5 bg-red-100 text-red-700 rounded-md text-sm hover:bg-red-200">Cancel</button></div></div>))}</div>}
                        </div>
                    </div>
                </motion.div>
                ...
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="bg-white/60 backdrop-blur-xl rounded-2xl shadow-lg border p-6">
                    ...
                    <div className="space-y-4">{(Array.isArray(leaderboard) ? leaderboard : []).map((user, index) => (<motion.div key={user._id} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: index * 0.05 }} className={`flex items-center space-x-4 p-4 rounded-xl ${user.isCurrentUser ? 'bg-blue-50 border-2 border-blue-200' : 'bg-gray-50'}`}><div className="flex-shrink-0 w-8 text-center">{getRankIcon(index)}</div><img src={user.image || `https://ui-avatars.com/api/?name=${user.displayName || user.email}`} alt={user.displayName || user.email} className="w-12 h-12 rounded-full object-cover" /><div className="flex-1"><div className="flex items-center space-x-2"><h3 className="font-semibold">{user.displayName || user.email}</h3>{user.isCurrentUser && (<span className="px-2 py-0.5 bg-blue-100 text-blue-800 text-xs rounded-full">You</span>)}</div><div className="flex flex-wrap gap-x-4 gap-y-1 text-sm text-gray-600"><span className="flex items-center space-x-1"><TrendingUp size={16} /><span>{user.totalWorkouts}</span></span><span className="flex items-center space-x-1"><Flame size={16} /><span>{user.totalCaloriesBurned.toLocaleString()}</span></span><span className="flex items-center space-x-1"><Calendar size={16} /><span>{user.weeklyWorkouts} this week</span></span></div></div></motion.div>))}</div>
                </motion.div>
                ...
                <AnimatePresence>{selectedFriend && ( <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setSelectedFriend(null)}><motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }} className="bg-white rounded-2xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-hidden flex flex-col" onClick={(e) => e.stopPropagation()}><div className="flex items-center justify-between p-6 border-b"><div className="flex items-center space-x-3"><img src={selectedFriend.friend.image || `https://ui-avatars.com/api/?name=${selectedFriend.friend.displayName || selectedFriend.friend.email}`} alt={selectedFriend.friend.displayName || selectedFriend.friend.email} className="w-12 h-12 rounded-full" /><div><h2 className="text-2xl font-bold">{selectedFriend.friend.displayName || selectedFriend.friend.email}'s Progress</h2><p className="text-gray-600">{selectedFriend.friend.email}</p></div></div><button onClick={() => setSelectedFriend(null)} className="p-2 hover:bg-gray-100 rounded-lg"><X/></button></div><div className="p-6 overflow-y-auto"><div className="grid grid-cols-1 md:grid-cols-3 gap-6"><div><h3 className="text-lg font-semibold mb-4">Recent Workouts</h3><div className="space-y-2">{(selectedFriend.workouts || []).slice(0,5).map(w => <div key={w._id} className="bg-gray-50 rounded-lg p-3"><h4 className="font-medium">{w.name}</h4><p className="text-sm text-gray-600">{new Date(w.workout_date).toLocaleDateString()}</p><p className="text-sm text-blue-600">{w.calories_burned} calories</p></div>)}{(!selectedFriend.workouts || selectedFriend.workouts.length === 0) && <p className="text-gray-500 text-sm">No workouts yet</p>}</div></div><div><h3 className="text-lg font-semibold mb-4">Recent Measurements</h3><div className="space-y-2">{(selectedFriend.measurements || []).slice(0,5).map(m => <div key={m._id} className="bg-gray-50 rounded-lg p-3"><p className="font-medium">{m.weight_kg} kg</p><p className="text-sm text-gray-600">{new Date(m.measurement_date).toLocaleDateString()}</p></div>)}{(!selectedFriend.measurements || selectedFriend.measurements.length === 0) && <p className="text-gray-500 text-sm">No measurements yet</p>}</div></div><div><h3 className="text-lg font-semibold mb-4">Recent Diet</h3><div className="space-y-2">{(selectedFriend.dietEntries || []).slice(0,5).map(e => <div key={e._id} className="bg-gray-50 rounded-lg p-3"><h4 className="font-medium">{e.food_name}</h4><p className="text-sm text-gray-600">{e.calories} calories</p><p className="text-sm text-gray-500">{new Date(e.entry_date).toLocaleDateString()}</p></div>)}{(!selectedFriend.dietEntries || selectedFriend.dietEntries.length === 0) && <p className="text-gray-500 text-sm">No diet entries yet</p>}</div></div></div></div></motion.div></motion.div> )}
                </AnimatePresence>
            </div>
        </Layout>
    );
