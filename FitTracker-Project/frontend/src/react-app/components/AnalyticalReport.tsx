  ...
  const processWeightData = () => {
    return measurements
      ...
      .map(m => ({
        ...
        weight: m.weight_kg,
        // BMI is not part of Measurement; compute on the fly if profile has height
        bmi:
          m.weight_kg && profile?.height_cm
            ? Number((m.weight_kg / Math.pow(profile.height_cm / 100, 2)).toFixed(2))
            : 0,
      }))
      .sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());
  };
  ...
  const processExerciseDistribution = () => {
    ...
    workouts.forEach(workout => {
      workout.exercises?.forEach(exercise => {
        const name = (exercise as any).exercise_name || (exercise as any).name || 'Unknown';
